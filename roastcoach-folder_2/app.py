...
def next_roast_plan(row, items):
    ...
    st.dataframe(shown[["clock", "control", "set to", "last time", "why"]].rename(
        columns={"clock": "time", "why": "why it changed"}),
        ...
        for change in moves_of(item):
            ...
            if change.get("from") is not None and change.get("to") is not None:
                if control == "drop":
                    ...
                    headline = (f"At **{change.get('clock', '—')}** — {control} "
                                f"**{float(change['from']):.0f} → {float(change['to']):.0f}**")
            else:
                headline = f"At **{change.get('clock', '—')}** — {control} "
...
def recipe_panel(row):
    ...
    table = (table.groupby(["clock", "at"], as_index=False)
             ...
             .sort_values("at"))
    st.dataframe(
        table[["clock", "power", "fan", "drum", "what"]].rename(
            columns={"clock": "time", "what": ""}),
        width="stretch", hide_index=True, height=min(420, 40 + 35 * len(table)))
    st.caption("Read straight from the roast's own recording — power, fan and drum move in "
               "whole steps, and this is where each one was set.")
...
def page_after():
    ...
    facts = st.columns(4)
    facts[0].metric("Coffee", str(row.get("coffee"))[:22])
    facts[1].metric("Green in", number(row.get("greenWeight"), " g", 0))
    facts[2].metric("Total", number(row.get("totalRoastMinutes"), " min"))
    facts[3].metric("Drop", number(row.get("drumDropTemperature"), " °C", 0))
...
def with_groupings(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in GROUP_BY.values():
        ...
        frame[column] = frame[column].fillna("unknown").replace("", "unknown").astype(str)
    return frame
