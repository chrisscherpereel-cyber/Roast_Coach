...
def page_roasts():
    ...
    table = pd.DataFrame({
        ...
        "Rating": view["rating"].astype("object").where(view["rating"].notna(), ""),
    })
...
def page_learning():
    ...
    table = pd.DataFrame({
        ...
        "Your measurement": learned["measured"].round(2).astype("object").where(
            learned["measured"].notna(), ""),
...
navigation = st.navigation([
    st.Page(page_coach, title="Coach", icon=":material/insights:", default=True),
